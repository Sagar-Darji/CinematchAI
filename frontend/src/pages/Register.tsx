...
function GoogleButton({ onAuth, disabled }: {
  ...
  const login = useGoogleLogin({
    onSuccess: async (resp) => {
      setLoading(true)
      try { onAuth(resp.access_token) } finally { setLoading(false) }
    },
    onError: () => {},
  })
  return (
...
export default function Register() {
  ...
  const handleRegister = async () => {
    ...
    try { _onSuccess(await registerUser(username.trim(), email.trim(), password)) }
  ...
  const handleGoogleAuth = async (accessToken: string) => {
    setLoading(true); setError('')
    try { _onSuccess(await googleAuth(accessToken)) }
    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-4" style={{ background: 'var(--bg-primary)' }}>
      <div className="w-full max-w-sm">
        <div className="text-center mb-10">
          ...
          <p className="mt-3 text-sm" style={{ color: 'var(--text-muted)' }}>Build your taste profile and get personalised recommendations.</p>
        </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          <input
            type="text" autoFocus placeholder="Username (3–32 chars, letters/numbers/_/-)"
          ...
          <button onClick={handleRegister} disabled={loading || !username.trim() || !email.trim() || !password}
          ...
          <p className="text-center text-sm mt-2" style={{ color: 'var(--text-muted)' }}>
            ...
            <Link to="/login" className="font-medium hover:underline" style={{ color: 'var(--accent-gold)' }}>Sign in</Link>
          </p>
        </div>
      </div>
    </div>
